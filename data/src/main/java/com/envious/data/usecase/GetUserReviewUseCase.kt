package com.envious.data.usecase

import com.envious.domain.model.Movie
import com.envious.domain.model.UserReview
import com.envious.domain.repository.MovieRepository
import com.envious.domain.usecase.BaseCaseWrapper
import com.envious.domain.util.Result
import javax.inject.Inject

class GetUserReviewUseCase @Inject constructor(
    private val repository: MovieRepository
) : BaseCaseWrapper<List<UserReview>, GetUserReviewUseCase.Params>() {

    override suspend fun build(params: Params?): Result<List<UserReview>> {
        if (params == null) throw IllegalArgumentException("Params should not be null")
        return repository.getUserReview(params.page, params.movieId)
    }

    class Params(val page: Int,val movieId: Int)
}
